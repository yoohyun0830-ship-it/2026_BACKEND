...
-- DML : 조작어 (1) INSERT 레코드삽입 (2) SELSECT 레코드조회 (3) UPDATE 레코드수정 (4) DELETE 레코드삭제
-- 레코드란? 테이블내 행/가로 단위

# [1] INSERT : INSERT INTO 테이블명( 속성명1 , 속성명2 ) VALUE( 값1 , 값2 )
INSERT INTO test( 번호 ,  이름 , 개수 ) value( 1 , "유재석" , 10 );
INSERT into test( 이름 , 개수 ) value("강호동" , 20 ); -- 번호 값 제외한 삽입
...
UPDATE test SET 개수 = 30 WHERE 이름 = "유재석"; -- 이름 속성값이 유재석이면 개수 속성 
-- 번호 속성값이 2이면 개수 속성값을 40 ,  이름 속성값을 강호동2로 수정
...
-- (DML)delete : 테이블내 레코드 삭제 [vs] truncate table : 테이블내 레코드 삭제 [vs] drop table : 테이블 자체 삭제
-- DDL은 취소 불가능 , DML(I/U/D)취소 DML은 취소(ROLLBACK) 가능
...
SELECT mid 회원아이디 FROM member 회원테이블; -- as를 생략하고 띄어쓰기 별칭

# [2] distinct , 조회결과 속성값 중복 제거
...
SELECT * FROM member WHERE mname != "블랙핑크"; -- 블랙핑크이면 반대(not부정)
SELECT * FROM member WHERE not mheight <= 162 ; -- '키'속성값이 162 이하이면
SELECT * FROM member WHERE mheight >= 165 and mheight <=170; -- '키' 속성값이 165~170 사이
SELECT * FROM member WHERE mheight BETWEEN 165 and 170; -- 속성명 between 시작값 asnd 끝값 , 사이/범위 값
