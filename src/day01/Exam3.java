    ...
    public static void main(String[] args) { // main start
        ...
        // 2. System.out.print(); 출력
         ...
         // %d : 정수가 들어갈 자리 , %f : 실수가 들어갈 자리
            ...
            // %6자릿수d : 자릿수 만큼 자리 차지 , 만일 비어있으면 0채움
            // %전체자릿수.소수자릿수f
         System.out.printf("\n저는 %s 이고 나이는 %d 입니다 \n", "유재석" , 40); // %s 에는 유재석 %d에는 40
         System.out.printf("\n저는 %s 이고 나이는 %6d 입니다 \n" , "유재석" , 40); // so + 엔터 -> println- > printf
         System.out.printf("\n저는 %s 이고 나이는 %-6d 입니다 \n" , "유재석" , 40); 
         System.out.printf("\n저는 %s 이고 나이는 %06d 입니다 \n" , "유재석" , 40);
         System.out.printf("\n저는 %6s 이고 점수는 %.2f 입니다" , "유재석" , 123.456789);

         // [3] 입력갹체/함수
         // 1. Scanner : 입력관련클래스 , Sc + 엔터 , * 코드파일 상단에 import java.util.scanner; (해당 폴더에서 클래스 가져옵니다.)
         // 2. Scanner : 아무거나, 카멜표기법 관례적으로 (클래스명은 대문자로 시작) 클래스명의 소문자로 변수명 설정
         // 3. 대입
         // 4. new : 인덱스화, 해당 클래스로 객체 만든다 뜻
         // 5. Scanner( System.in ) : 생성자(클래스명동일) 안에 시스템입력(System.in 객체)
         // * 클래스 수렴 전까지는 해당 하는 한줄의 코드 복붙하자.
