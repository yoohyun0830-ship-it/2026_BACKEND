...
/*[문제 2] String 타입의 데이터를 3개 저장할 수 있는 
배열 season을 new 키워드를 사용하여 생성하세요.
...
/*[문제 3] 다음과 같은 과일 배열이 있습니다. 
for 반복문을 사용하여 배열의 모든 요소를 하나씩 콘솔에 출력하시오.
...
// 배열변수명.length : 배열내 요소총길이 , 자동타입변환 : 연산시 두 항의 자료타입중 더 큰 타입으로 결과
...
/*[문제 6] 다음 배열에서 'A'형 혈액형을 가진 사람이 몇 명인지 
...
for(int index = 0 ; index <= products.length -1 ; index++){
    if(상품명.equals(products[index])){ // [1] 입력받은 상품명과 index 번째 상품명과 같으면
...
for(int index = 0 ; index <= carNumbers.length-1; index++){
    ...
    // [2] 주차(분) 출력
    System.out.println(usageMinutes[index]+"분 ");
    // [3] 주차요금 출력
    int fee = 1000;
